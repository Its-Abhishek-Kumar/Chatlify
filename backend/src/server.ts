...
io.on("connection", async (socket) => {
  ...
  try {
    ...
    const directConvIds = directConversations.map((c: any) => c._id);
    ...
    if (offlineMessages.length > 0) {
      const msgIds = offlineMessages.map((m: any) => m._id);
      await Message.updateMany({ _id: { $in: msgIds } }, { status: "delivered" });

      // Notify senders about delivery status updates
      const senders = new Set(offlineMessages.map((m: any) => m.senderId?.toString() || ""));
      senders.forEach(senderId => {
        if (!senderId) return;
        const senderMsgs = offlineMessages.filter((m: any) => m.senderId?.toString() === senderId);
        senderMsgs.forEach((msg: any) => {
  ...
  socket.on("conversation:read", async ({ conversationId }) => {
    try {
      ...
      if (conversation.type === "direct") {
        ...
        const otherParticipant = conversation.participants.find((p: any) => p.toString() !== userId.toString());
        if (otherParticipant) {
    ...
    try {
      ...
      if (deleteFor === "everyone") {
        ...
        for (const p of conversation.participants) {
          ...
          convObj.participants = (convObj.participants || []).filter((pEl: any) => pEl != null);
