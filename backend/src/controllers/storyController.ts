...
export const getActiveStories = async (req: any, res: any) => {
  try {
    const userId = req.userId;
    ...
    const participantIds = new Set<string>([userId.toString()]);
...
// Like / unlike a story
export const toggleLikeStory = async (req: any, res: any) => {
  try {
    const { storyId } = req.params;
    const userId = req.userId;
