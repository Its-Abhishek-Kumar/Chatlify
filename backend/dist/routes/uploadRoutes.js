...
const encodeContentDispositionName = (name) => {
...
const encodeInlineDispositionName = (name) => {
    const safeFallback = name.replace(/[^\w.-]/g, "_") || "attachment";
...
const isCloudinaryUrl = (url) => url.hostname.includes("res.cloudinary.com");
const cloudinaryRawCandidate = (url) => {
...
router.get("/file/:storedName", async (req, res) => {
    try {
        ...
        res.setHeader("Content-Disposition", disposition === "attachment" ? encodeContentDispositionName(originalName) : encodeInlineDispositionName(originalName));
