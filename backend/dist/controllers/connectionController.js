...
export const respondConnectionRequest = async (req, res) => {
    try {
        ...
        if (action === "accept") {
            ...
            await ConnectionNotification.updateMany({ userId: userId, senderId: connection.requester, type: "request" }, { isRead: true });
...
export const markNotificationsRead = async (req, res) => {
    try {
        const userId = req.userId;
        await ConnectionNotification.updateMany({ userId: userId, isRead: false }, { isRead: true });
