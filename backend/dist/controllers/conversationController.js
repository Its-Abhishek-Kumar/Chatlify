...
const findOrCreateDirectConversation = async (currentUserId, userId) => {
...
export const createOrGetConversation = async (req, res) => {
    try {
        ...
        const currentUserId = req.userId;
        const conversation = await findOrCreateDirectConversation(currentUserId, userId);
...
export const createGroupConversation = async (req, res) => {
    try {
        ...
        const uniqueParticipants = Array.from(new Set([currentUserId, ...(participantIds || [])]));
...
export const addConversationMembers = async (req, res) => {
    try {
        ...
        const newParticipants = Array.from(new Set([...conversation.participants.map((p) => p.toString()), ...(participantIds || [])]));
...
export const removeConversationMembers = async (req, res) => {
    try {
        const { conversationId, participantIds } = req.body;
        const currentUserId = req.userId;
        const conversation = await Conversation.findById(conversationId);
        ...
        const newParticipants = conversation.participants.filter((p) => !toRemove.has(p.toString()));
    ...
    try {
        ...
        if (conversation.admin && getObjectIdStr(conversation.admin) === getObjectIdStr(currentUserId)) {
            if (newParticipants.length > 0) {
                ...
                conversation.admin = getObjectIdStr(newParticipants[0]);
            }
            else {
